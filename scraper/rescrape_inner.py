"""
Rescrapes Google Maps for cafes across Melbourne's inner suburbs — CBD, Fitzroy,
Brunswick, Collingwood, Carlton, Richmond, Northcote, and nearby streets.
Adds any missing cafes to public/cafes.json with images on Cloudinary.
No API key needed.
"""
...
FOUND_FILE = ROOT / "data" / "inner_rescrape_candidates.json"
...
SEARCHES = [
    # CBD
    ("cafe Melbourne CBD 3000", "Melbourne"),
    ("coffee Melbourne CBD", "Melbourne"),
    ("cafe Flinders Lane Melbourne", "Melbourne"),
    ("cafe Collins Street Melbourne CBD", "Melbourne"),
    ("cafe Bourke Street Melbourne CBD", "Melbourne"),
    ("cafe Lonsdale Street Melbourne CBD", "Melbourne"),
    ("cafe Swanston Street Melbourne", "Melbourne"),
    ("cafe Little Bourke Street Melbourne", "Melbourne"),
    ("cafe Degraves Street Melbourne", "Melbourne"),
    ("cafe Hardware Lane Melbourne", "Melbourne"),
    ("cafe QVM Queen Victoria Market Melbourne", "Melbourne"),
    ("dessert cafe Melbourne CBD", "Melbourne"),
    ("specialty coffee Melbourne CBD", "Melbourne"),
    # Fitzroy
    ("cafe Fitzroy Melbourne", "Fitzroy"),
    ("cafe Brunswick Street Fitzroy", "Fitzroy"),
    ("cafe Smith Street Fitzroy", "Fitzroy"),
    ("cafe Johnston Street Fitzroy", "Fitzroy"),
    ("cafe Gertrude Street Fitzroy", "Fitzroy"),
    ("specialty coffee Fitzroy Melbourne", "Fitzroy"),
    # Collingwood
    ("cafe Collingwood Melbourne", "Collingwood"),
    ("cafe Smith Street Collingwood", "Collingwood"),
    ("cafe Johnston Street Collingwood", "Collingwood"),
    # Brunswick
    ("cafe Brunswick Melbourne", "Brunswick"),
    ("cafe Sydney Road Brunswick", "Brunswick"),
    ("cafe Lygon Street Brunswick", "Brunswick"),
    ("cafe Glenlyon Road Brunswick", "Brunswick"),
    ("specialty coffee Brunswick Melbourne", "Brunswick"),
    # Carlton
    ("cafe Carlton Melbourne", "Carlton"),
    ("cafe Lygon Street Carlton", "Carlton"),
    ("cafe Grattan Street Carlton", "Carlton"),
    ("cafe Elgin Street Carlton", "Carlton"),
    # Brunswick East
    ("cafe Brunswick East Melbourne", "Brunswick East"),
    ("cafe Lygon Street Brunswick East", "Brunswick East"),
    # Fitzroy North
    ("cafe Fitzroy North Melbourne", "Fitzroy North"),
]
...
async def get_place_details(page, name, suburb):
    ...
    skip_types = ["petrol", "service station", "supermarket", "pharmacy", "gym",
                  "fitness centre", "newsagent", "hardware", "bottle shop"]
    ...
    if addr_m:
        ...
        # Validate postcode matches suburb
        pc_m = re.search(r"VIC\s+(\d{4})", result["address"])
        if pc_m:
            valid_pcs = SUBURB_POSTCODES.get(suburb, [])
            if valid_pcs and pc_m.group(1) not in valid_pcs:
                result["_skip"] = True
                return result
            ...
            return Array.from(document.querySelectorAll('img'))
                ...
                .filter(function(s) {
                    return s && s.includes('googleusercontent.com')
                        && !s.match(/=[ws][1-4][0-9](-h[0-9]+)?$/)
                        && s.length > 60;
                });
        })()""")
        cleaned = []
        for u in imgs[:MAX_IMAGES * 2]:
...
async def main():
    ...
    inner_counts = {}
    for c in cafes:
        if c["suburb"] in SUBURB_POSTCODES:
            inner_counts[c["suburb"]] = inner_counts.get(c["suburb"], 0) + 1
    print("Current counts:")
    for s, n in sorted(inner_counts.items()):
        print(f"  {s}: {n}")

    # Phase 1: collect all candidates
    all_candidates: dict[str, dict] = {}  # name -> {href, suburb}
    async with async_playwright() as p:
        ...
        for query, suburb in SEARCHES:
            items = await scroll_and_extract(page, query)
            new = 0
            for item in items:
                name = item["name"]
                if name and name not in all_candidates:
                    all_candidates[name] = {"href": item["href"], "suburb": suburb}
                    new += 1
            print(f"  [{suburb}] '{query}': {len(items)} results, {new} new")
        ...
        for i, (name, data) in enumerate(missing):
            ...
            try:
                ...
                details = await get_place_details(page, name, suburb)
                ...
                print(f"  hours:  {len(entry['openingHours'])} days  images: {len(images)}")
